        ...
        while (true) {
           ...
           switch (response) {
               case "a": 
                 ...
                 if (!(cart.add(item))) {
                     System.out.println(item.getItem() + " is already in your shopping cart.");
                 } else {
                     System.out.println(item.getItem() + " was added to your shopping cart.");
                 }
                 break;
               case "b":
                 if (cart.isEmpty()) {
    ...
    public static void loadItems(String fileName) throws FileNotFoundException {
        ...
        for (int i = 0; fileScanner.hasNextLine(); i++) {
            ...
            for (int j = 0; j < parsedLine.length; j++) {
                String[] parsedItem = parsedLine[j].split("=");
                Item item = new Item(parsedItem[0], Double.parseDouble(parsedItem[1]));
